...
async function createssh(username, password, exp, iplimit, serverId) {
  ...
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
      ...
      const param = `:5888/createssh?user=${username}&pass=${password}&exp=${exp}&quota=${quota}&iplimit=${iplimit}&auth=${auth}`;
...
async function createvmess(username, exp, quota, limitip, serverId) {
  ...
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
      ...
      const domain = server.domain;
      const auth = server.auth;
      const param = `:5888/createvmess?user=${username}&exp=${exp}&quota=${server.quota}&iplimit=${limitip}&auth=${auth}`;
      const url = `http://${domain}${param}`;
      axios.get(url)
        .then(response => {
          if (response.data.status === "success") {
            const vmessData = response.data;
            const msg = `
🌟 *AKUN VMESS PREMIUM* 🌟
...
│ Quota: \`${vmessData.quota_gb === '0 GB' ? 'Unlimited' : vmessData.quota_gb}\`
  ...
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
      ...
      axios.get(url)
        .then(response => {
          if (response.data.status === "success") {
            const trojanData = response.data;
            const msg = `
🌟 *AKUN TROJAN PREMIUM* 🌟
